...
public class ClientKeepAliveFactoryImpl implements KeepAliveMessageFactory {
    ...
    public boolean isRequest(IoSession session, Object message) {
        ...
        byte[] m = (byte[])message;
        log.debug("【银联支付：客户端收到服务端心跳请求 sessionId： " + session.getId() + "， message：" + new String(m) + "】");
        return false;
    }
    ...
    public boolean isResponse(IoSession session, Object message) {
        ...
        byte[] m = (byte[])message;
        log.debug("【银联支付：客户端发送心跳后收到服务端反馈 sessionId： " + session.getId() + "， message：" + new String(m) + "】");
        if (message.equals(UnionPayCons.HEARTBEAT_RESP)) {
            return true;
        } else {
            return false;
        }
    ...
    public Object getRequest(IoSession session) {
        // 获取心跳请求包
        try {
            return UnionPayCons.HEARTBEAT_REQ.getBytes(UnionPayCons.charCode);
        } catch (UnsupportedEncodingException e) {
            log.debug("【银联支付：客户端心跳0000转换byte异常 sessionId： " + session.getId()  + "】");
            e.printStackTrace();
            return null;
        }
    }
